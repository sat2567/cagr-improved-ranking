...
DEFAULT_TOP_N = 5
DEFAULT_RISK_FREE = 0.06           # Default 6% annual
...
def calculate_sortino_ratio(returns, daily_rf_rate):
...
def calculate_flexible_momentum(series, w_3m, w_6m, w_12m, use_risk_adjust=False):
    ...
    IMPROVED: Uses a 5-day moving average for prices to reduce noise.
    """
    if len(series) < 70: return np.nan 
    
    # Smooth the series (5-day simple moving average) to avoid single-day outliers
    smoothed = series.rolling(window=5).mean()
    
    # Get the last available smoothed price
    ...
    def get_past_price(days_ago):
        ...
        # Find data up to target date
        sub_series = smoothed[smoothed.index <= target_date]
...
def calculate_calmar_ratio(series):
    ...
    # IMPROVED: Handle Zero Drawdown or NaN cases
    if np.isnan(cagr) or np.isnan(max_dd): return np.nan
    if max_dd == 0: return np.nan # Cannot divide by zero (perfect fund?)
        ...
        if strategy_type != 'custom':
             for col in nav_wide.columns:
                ...
                if strategy_type == 'momentum':
                    ...
                    val = calculate_flexible_momentum(series, w3, w6, w12, momentum_config['risk_adjust'])
                else:
                    ...
                    if strategy_type == 'sharpe': val = calculate_sharpe_ratio(rets, daily_rf)
                    elif strategy_type == 'sortino': val = calculate_sortino_ratio(rets, daily_rf)
            ...
            for col in nav_wide.columns:
                ...
                if custom_weights.get('sharpe', 0) > 0: row['sharpe'] = calculate_sharpe_ratio(rets, daily_rf)
                if custom_weights.get('sortino', 0) > 0: row['sortino'] = calculate_sortino_ratio(rets, daily_rf)
                ...
                if required_bench and bench_rets is not None and not bench_rets.empty:
                    if custom_weights.get('alpha', 0) > 0 or custom_weights.get('beta', 0) > 0 or custom_weights.get('treynor', 0) > 0:
                        capm_metrics = calculate_alpha_beta_treynor(rets, bench_rets, daily_rf, annual_rf)
            ...
            if temp_metrics:
                ...
                if 'max_dd' in metrics_df.columns:
                    # max_dd is negative (e.g., -0.20), so larger value (-0.10) is better. Standard rank(ascending=True) works.
...
def generate_snapshot_table(nav_wide, analysis_date, holding_days, strategy_type, names_map, config, custom_weights=None, momentum_config=None, benchmark_series=None):
    ...
    for col in nav_wide.columns:
        ...
        if strategy_type == 'sharpe': row['Score'] = calculate_sharpe_ratio(rets, daily_rf)
        elif strategy_type == 'sortino': row['Score'] = calculate_sortino_ratio(rets, daily_rf)
    ...
    # Define metrics and direction OUTSIDE the conditional block to prevent UnboundLocalError
    ranking_directions = {
...
def main():
    ...
    risk_adjust_mom = st.sidebar.checkbox("Risk Adjust Momentum?", value=True)
    ...
    def display_strategy_results(strat_type, c_weights, nav_data, names_map, top_n, holding_days, config, momentum_config, nifty_data):
        ...
        if sel_date_str:
            ...
            df = generate_snapshot_table(nav_data, sel_date, holding_days, strat_type, names_map, config, c_weights, momentum_config, nifty_data)
    ...
    with tab_custom:
        with st.form("custom_strat_form"):
            ...
            c1, c2, c3 = st.columns(3)
            w_sharpe = c1.slider("Sharpe Weight", 0, 100, 20, 10, key="s_sharpe")
            w_sortino = c2.slider("Sortino Weight", 0, 100, 0, 10, key="s_sortino")
            w_mom = c3.slider("Momentum Weight", 0, 100, 20, 10, key="s_mom")

            c4, c5 = st.columns(2)
            w_ir = c4.slider("Info Ratio Weight", 0, 100, 0, 10, key="s_ir")
            w_calmar = c5.slider("Calmar Weight", 0, 100, 20, 10, key="s_calmar")

            st.markdown("##### Manager Skill / Systematic Risk")
            c6, c7, c8 = st.columns(3)
            w_alpha = c6.slider("Alpha Weight", 0, 100, 20, 10, key="s_alpha")
            w_treynor = c7.slider("Treynor Weight", 0, 100, 0, 10, key="s_treynor")
            w_beta = c8.slider("Beta Weight (Lower is Better)", 0, 100, 10, 10, key="s_beta")

            st.markdown("##### Pure Risk Metrics")
            c9, c10 = st.columns(2)
            w_vol = c9.slider("Low Volatility Weight", 0, 100, 10, 10, key="s_vol")
            w_max_dd = c10.slider("Max Drawdown Weight", 0, 100, 0, 10, key="s_maxdd")
